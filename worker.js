...
<body>
  <div class="card">
    ...
    <div class="logo-box">
      <img src="/payed.png" alt="Dolr Pay Logo">
    </div>
...
function normalizeEdfaStatus(obj = {}) {
  ...
  const successStates = ["APPROVED", "SUCCESS", "PAID", "CAPTURED", "OK", "COMPLETED"];
  const failedStates = ["DECLINED", "FAILED", "ERROR", "CANCELED", "CANCELLED", "VOIDED"];
...
function renderResult(initialStatus, orderId) {
  ...
  const noteText = isSuccess
    ? "تم تنفيذ العملية بنجاح. شكرًا لاستخدامك Dolr Pay."
    : isFailed
      ? "نعتذر، فشلت عملية الدفع. لن يتم خصم أي مبلغ في حال الفشل."
      : "جاري التحقق من حالة العملية من بوابة الدفع EdfaPay...";

  const iconSVG = isSuccess
    ? `<svg viewBox="0 0 150 150"><circle cx="75" cy="75" r="60" fill="#0f172a" stroke="#4ade80" stroke-width="8"/><path d="M50 78 68 96 105 55" fill="none" stroke="#4ade80" stroke-width="10" stroke-linecap="round" stroke-linejoin="round"/></svg>`
    : isFailed
      ? `<svg viewBox="0 0 150 150"><circle cx="75" cy="75" r="60" fill="#0f172a" stroke="#f97373" stroke-width="8"/><path d="M55 55 95 95 M95 55 55 95" fill="none" stroke="#f97373" stroke-width="10" stroke-linecap="round"/></svg>`
      : `<svg viewBox="0 0 150 150"><rect x="45" y="20" width="60" height="20" rx="6" fill="#0f172a" stroke="#38bdf8" stroke-width="5"/><rect x="45" y="110" width="60" height="20" rx="6" fill="#0f172a" stroke="#38bdf8" stroke-width="5"/><path d="M55 40h40c-2 12-10 18-20 24-10-6-18-12-20-24z" fill="#e5e7eb" stroke="#38bdf8" stroke-width="3"/><path d="M55 110h40c-2-12-10-18-20-24-10 6-18 12-20 24z" fill="#e5e7eb" stroke="#38bdf8" stroke-width="3"/></svg>`;
...
<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
@font-face { font-family: "Thmanyah sans 1.2"; src: url("/Thmanyahsans12-Bold.ttf") format("truetype"); font-weight: 700; }
* { box-sizing: border-box; }
body { margin: 0; padding: 0; height: 100vh; background: radial-gradient(circle at 5% 5%, #22d3ee33, transparent 70%), radial-gradient(circle at 95% 95%, #a855f733, transparent 70%), #010512; display: flex; justify-content: center; align-items: center; font-family: "Thmanyah sans 1.2", sans-serif; color: #f1f5f9; }
.box { width: 92%; max-width: 500px; padding: 34px 30px 28px; text-align: center; background: rgba(15, 23, 42, 0.82); border-radius: 26px; border: 1px solid rgba(148,163,184,0.25); backdrop-filter: blur(25px); box-shadow: 0 0 40px rgba(34,211,238,0.55), 0 0 80px rgba(168,85,247,0.45), inset 0 0 20px rgba(255,255,255,0.06); animation: fadeUp .8s ease-out; }
@keyframes fadeUp { from { opacity: 0; transform: translateY(25px); } to { opacity: 1; transform: translateY(0); } }
.logo { width: 95px; margin-bottom: 16px; }
.brand { font-size: 19px; margin-bottom: 8px; letter-spacing: 0.5px; color: #e2e8f0; }
.status-icon { width: 130px; margin: 10px auto 14px; filter: drop-shadow(0 0 8px ${color}33); }
.title { font-size: 26px; font-weight: 700; margin-bottom: 10px; color: ${color}; text-shadow: 0 0 12px ${color}45; }
.note { margin-top: 10px; font-size: 14px; color: #cbd5e1; line-height: 1.7; }
button { margin-top: 26px; padding: 14px 30px; border-radius: 999px; border: none; background: linear-gradient(135deg, #22d3ee, #0ea5e9, #a855f7); color: #0f172a; font-size: 16px; font-weight: 700; cursor: pointer; box-shadow: 0 0 22px rgba(34,211,238,0.65); transition: 0.2s ease; }
button:hover { transform: scale(1.05); }
</style>
</head>
<body>
  <div class="box">
    <img src="/payed.png" class="logo" alt="Dolr Pay">
    <div class="brand">Dolr Pay – Secure Payment</div>
...
(async function() {
  ...
  setTimeout(() => {
    window.location.href = "/callback?final=1&order_id=" + encodeURIComponent(orderId);
  }, 8000);
})();
</script>
</body>
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/Thmanyahsans12-Bold.ttf") {
      return new Response(await env.ASSETS.get("Thmanyahsans12-Bold.ttf", "arrayBuffer"), { headers: { "Content-Type": "font/ttf" } });
    }
    ...
    if (url.pathname === "/api/create-payment") {
      try {
        ...
        if (!merchantId || !merchantPassword) {
          return new Response(JSON.stringify({ error: "Missing merchant info" }), { status: 500 });
        }

        const rawPhone = String(body.phone || "").trim();
        if (!/^[0-9]{12}$/.test(rawPhone)) {
          return new Response(JSON.stringify({ error: "صيغة رقم الجوال غير صحيحة" }), { status: 400 });
        }
        
        const email = String(body.email || "").trim() || "none@example.com";
        let clientIp = request.headers.get("CF-Connecting-IP") || "127.0.0.1";

        const hash = buildHash(orderId, amount, currency, description, merchantPassword);
        ...
        form.append("payer_phone", rawPhone);
        ...
        return new Response(JSON.stringify({ error: "internal", details: err.toString() }), { status: 500 });
      }
    }
    ...
    if (url.pathname === "/callback") {
      try {
        ...
        if (contentType.includes("form")) {
          ...
          raw.split("&").forEach(p => { const [k,v] = p.split("="); if(k) payload[k] = decodeURIComponent(v||""); });
        }

        const status = normalizeEdfaStatus(payload);
        const orderId = payload.order_id || payload.order_number || "N/A";
